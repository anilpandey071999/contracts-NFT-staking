...
import { BaseStakingV2U, ERC20Test } from "../../typechain-types";
...
describe("Base Staking V2 Upgradeable", function () {
  ...
  beforeEach(async () => {
    const Staking = await ethers.getContractFactory("BaseStakingV2U");
  ...
  describe("Upgrades", () => {
    it("should not let non-admin upgrade contract", async () => {
      const V2 = await ethers.getContractFactory(
        "BaseStakingV2UTestUpgrade",
        nonAdmin
      );
      await expect(
        upgrades.upgradeProxy(staking.address, V2)
      ).to.be.revertedWith("Ownable: caller is not the owner");
    });

    it("should let admin upgrade staking contract", async () => {
      const V2 = await ethers.getContractFactory("BaseStakingV2UTestUpgrade");
      const upgraded = await upgrades.upgradeProxy(staking.address, V2);

      const value = await upgraded.upgraded();
